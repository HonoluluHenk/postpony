...
- The coverage gate is **>= 90% for all metrics** (statements, branches, functions, lines) on a full `npm run test`. When a change drops a metric below the gate, close the gap with tests for the changed code before committing — not by chasing unrelated legacy files.
