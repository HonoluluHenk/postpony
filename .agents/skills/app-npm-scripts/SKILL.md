...
| `npm run e2e-baseline`     | `run-s -n e2e-baseline:*`                     | Build, run Playwright, and regenerate all screenshot baselines; local maintenance only.                                                                                                                                                                               |
...
- `npm run e2e-baseline` builds the app and regenerates **all** screenshot baselines. Review every PNG diff and commit only intentional changes. It is local maintenance only; `npm run e2e` and `npm run verify` compare against committed baselines.
- `-n` means the baseline E2E task still starts if the build task fails. Check the aggregate command's exit status, then run `npm run verify` before finishing.
