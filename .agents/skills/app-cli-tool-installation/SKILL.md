...
|----------|-------------------|------------------------|---------------------------------------------------------|
| `node`   | `26`              | `26.9.0`               | Runtime for the Hono server and all npm scripts.        |
...
Note: `wrangler` (Cloudflare Worker deploy/validate: `worker:build`,
`worker:deploy`, `secret put`, `types`) is a `devDependency` in
`package.json`, resolved from `node_modules/.bin` — the default choice
for project-local JS CLIs.
...
- Prefer `package.json` for project-local JS dependencies (Vitest,
  Playwright, TypeScript, Vite, wrangler, etc.); use `mise` for
  system-level CLIs and language runtimes. `mise`'s `npm:` backend is
  reserved for CLIs that need to be available outside an `npm install`
  (e.g. in a bare shell or CI step that has not run `npm ci`).
