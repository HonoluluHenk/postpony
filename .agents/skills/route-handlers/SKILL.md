...
- Add or modify a route handler (`*-get.ts` / `*-post.ts`) or its `.tsx` view component.
...
- `app.render(component)` — renders a `JSX.Element` to an HTML string via `.toString()`. The `ViewContext` (`t`, `locale`, `languageOptions`, `inputFormat`, `isPartial`, `baseUrl`) is available via `app.view` and passed as props to components. Use `pageLayout(view, content, title?)` from `src/routes/layouts/main.tsx` to handle partial vs full-page rendering. See [ADR 0019](../../../docs/adr/0019-jsx-templates.md).
...
`onError` handler in `src/index.ts` turns them into the right UI response (an out-of-band `#error-container` for HTMX partials, or the error page):
...
- File naming: handlers and their JSX components typically share a `.tsx` file (e.g. `create-get.tsx`, `edit-id-get.tsx`); shared partials are separate `.tsx` files in the same directory.
- **Partial vs initial render**: the edit page's initial template (`edit.tsx`)
  must render every UI element that the HTMX partials (`proposed-dates-section.tsx`,
  `team-section.tsx`, etc.) render. Tests load the page fresh — they never run an HTMX request first. If a toggle or control only exists in the partial, it is invisible on page load.
