...
- Screenshot baseline regeneration is local-only: `npm run e2e-baseline` builds and runs Playwright with `--update-snapshots all`. CI keeps using `npm run e2e` to compare against committed baselines.
