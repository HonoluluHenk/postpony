...
`npm run verify` (lint → test → build → e2e) remains the manual local gate. Visual baseline regeneration is also local-only: `npm run e2e-baseline` builds and runs Playwright with `--update-snapshots all`; CI and `verify` compare against committed baselines and never rewrite them. `npm run check:actionlint` runs `actionlint` over the workflow YAML but is deliberately not wired into `verify` or CI — the script is named `check:` instead of a `lint:actionlint` so the repo's `lint:*` run-all wildcard cannot auto-wire it into the gate. The workflow is validated by running it, not by linting it in the gate (ADR-0028).
...
- **`deploy-prod`** (needs `unit` + `e2e`, only `v*` tag pushes): the Cloudflare credentials (`CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID`) are the same GitHub secrets as staging; the Turso token comes from `TURSO_DB_AUTH_TOKEN` (staging uses `TURSO_DB_AUTH_TOKEN_STAGING`). The tag/version guard first refuses the deploy (exit non-zero, before any account operation) unless the pushed tag equals `v${npm_package_version}` at the tagged commit — belt-and-suspenders against the release script, which produces matching tag/manifest pairs from its side. On match it refreshes the production Worker's `TURSO_DB_AUTH_TOKEN` secret from the GitHub secret (fail-loud if missing) and runs `npm run worker:deploy` (no `--env` → the top-level `postpony` config, custom domain `spielverlegung.date`). No migration step: the worker migrates lazily on first request (§7.2).
