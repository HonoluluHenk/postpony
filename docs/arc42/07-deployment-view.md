...
- **`deploy-staging`** (needs `unit` + `e2e`, only `main` merges and `workflow_dispatch`): dry-runs the staging bundle (`wrangler deploy --dry-run --env staging`), refreshes the staging Worker's `TURSO_DB_AUTH_TOKEN` secret from the GitHub secret (fail-loud if missing), then `npm run worker:deploy -- --env staging` → `postpony-staging`.
- **`deploy-prod`** (needs `unit` + `e2e`, only `v*` tag pushes): the tag/version guard first refuses the deploy (exit non-zero, before any account operation) unless the pushed tag equals `v${npm_package_version}` at the tagged commit — belt-and-suspenders against the release script, which produces matching tag/manifest pairs from its side. On match it refreshes the production Worker's `TURSO_DB_AUTH_TOKEN` secret from the GitHub secret (fail-loud if missing) and runs `npm run worker:deploy` (no `--env` → the top-level `postpony` config, custom domain `spielverlegung.date`). No migration step: the worker migrates lazily on first request (§7.2).
