...
run through `runOpponentCommand` in `run-opponent-command.ts`. Both builders read the organizing/opponent team's ranked bands from `PostponementRules.availabilityRanking`; `sort-control.tsx` turns those bands into labelled rail groups via `groupByAvailabilityBands`/`availabilityGroupLabel` (Full strength / With if-necessary / Reduced strength / Not playable, ADR-0027).
