...
2. **Create New ReSchedule**: The Team Captain creates the postponement via the scrape wizard.
...
In this step, the Admin (Team Captain) enters all relevant constraints and information required for the Scheduling Engine to suggest possible dates and times.
...
2. **Input Information**: The Admin provides the following data (as defined in
    ...
    * **Venue Booking**: Specific dates and times when the venue is already occupied.
...
3. **Save/Proceed**: The system saves the entered information and prepares to generate suggestions or move to the invitation phase.
4. **Generate Suggestions**: (Optional) The user triggers the suggestion engine to see possible dates and times based on current inputs.
...
3. **Generate Text**: The system generates a pre-formatted message including:
    ...
    * A direct **Invitation Link** (which implicitly includes access rights).
4. **Copy/Share**:
...
This use case describes how participants (home team players, opponent captain, and opponent players) interact with the Reschedule session after receiving an invitation.
...
5. **Propose New Date & Time**:
    * If the participant finds the existing proposals unsuitable or has better suggestions, they can propose a new date and time.
    * The system allows the participant to select a slot and add it to the list of proposed dates and times for others to see/vote on.
...
2. **View Venue**: The system displays a list of existing venues for the club.
3. **Add/Edit Venue**:
...
4. **Manage Availability**:
    ...
    * The user can enter **Blackout Date/Time** (Booking) where the venue is unavailable.
...
1. **Manage Team**: The Club Manager selects "Manage Teams" from the club dashboard.
2. **Add/Edit Team**:
