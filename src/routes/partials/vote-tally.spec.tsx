...
describe('VoteTally component', () => {
  ...
  it('renders heading level 3 by default with default title and id', () => {
    ...
    expect(html)
      .toContain('<h3 id="vote-tally-title">Vote Summary</h3>');
    expect(html)
      .toContain('<caption class="visually-hidden">Vote Summary</caption>');
  });
  ...
  it('supports custom heading-level variants and custom titleId/title', () => {
    ...
    expect(htmlH2)
      .toContain('<h2 id="custom-h2-title">Custom Summary</h2>');
    expect(htmlH2)
      .toContain('<caption class="visually-hidden">Custom Summary</caption>');
  ...
  it('wraps the tally in a closed details with the heading inside the summary when disclosure is set', () => {
    ...
    expect(html)
      .toContain('</details>');
    expect(html)
      .toMatch(/<summary>\s*<h3 id="vote-tally-title">Vote Summary<\/h3>\s*<\/summary>/);
  ...
  it('renders table headers and tally values for each date', () => {
    ...
    expect(html)
      .toContain('<th scope="col" class="num">Yes</th>');
    expect(html)
      .toContain('<th scope="col" class="num">if necessary</th>');
    expect(html)
      .toContain('<th scope="col" class="num">No</th>');

    expect(html)
      .toContain('<td data-label="Proposed Date &amp; Time">10.10.2026 19:00</td>');
    expect(html)
      .toContain('<td data-label="Yes" class="num">3</td>');
    expect(html)
      .toContain('<td data-label="if necessary" class="num">1</td>');
    expect(html)
      .toContain('<td data-label="No" class="num">0</td>');

    expect(html)
      .toContain('<td data-label="Proposed Date &amp; Time">12.10.2026 20:00</td>');
    expect(html)
      .toContain('<td data-label="Yes" class="num">1</td>');
    expect(html)
      .toContain('<td data-label="if necessary" class="num">2</td>');
    expect(html)
      .toContain('<td data-label="No" class="num">2</td>');
  });
});
