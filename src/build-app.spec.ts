...
describe('classifyError', () => {
  const t: (key: TranslationKeys) => string = (key) => createApp().t(key);
...
describe('buildApp error handling', () => {
  ...
  it('maps a ClickTTError to a 400 with the localized scrape message', async () => {
    ...
    expect(await res.text()).toContain('click-tt.ch is currently reporting an error');
  });
  ...
  it('maps an HTTPException to its status and message on a full page', async () => {
    ...
    expect(res.status).toBe(409);
    expect(await res.text()).toContain('teapot declared');
  });
