...
- [ ] arc42 reconciled per the `app-arc42-docs` skill: toolchain/CI constraint added, coverage row now says machine-enforced, staging worker + pipeline in the deployment view, risk of "no CI" resolved and debt row dropped, README verified-commit bumped.
- [ ] `CONTEXT.md` unchanged: CI/CD terms are engineering infra, not postponement domain vocabulary.
