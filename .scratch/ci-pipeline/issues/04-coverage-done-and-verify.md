...
- [ ] `npm run test` passes with the existing thresholds (no threshold changes unless something genuinely regressed).
- [ ] Any stale in-repo references implying coverage is unenforced or a live future risk are reconciled (arc42 rows fold into ticket 07; this ticket covers non-arc42 references).
- [ ] This issue is closed out as verified, not re-implemented.
