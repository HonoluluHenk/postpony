...
- `4a281b4` review-fixed: story-1 gaps, indentation, e2e coverage — after the code-review pass: toggle-votable (Voting enabled/disabled), reopen (Postponement reopened), and refresh-clashes (Schedule check refreshed) now emit short polite status announcements (new en/de keys); confirm-date with clashes announces only the clash warning via the status element and the visible warning paragraph lost `role="alert"` (no double announcement); handler specs assert each new status message; indentation/EOF-newline nits fixed; e2e added for status-after-delete and language-select ≥24px.
